...
#include "../../include/cplex/CPLEX_LP_IMSTSolver_v3.hpp"
...
const static log4cxx::LoggerPtr logger(
		log4cxx::Logger::getLogger("cplex.CPLEX_LP_IMSTSolver_v3"));
...
void CPLEX_LP_IMSTSolver_v3::buildModel(IncrementalParam k) {
	INFO_NOARG(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL);
...
void CPLEX_LP_IMSTSolver_v3::generateMoveCommodityFromSourceConstraints() {
	INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_MOVES_SRC,
			initialVertexIdx);
	for (auto& nodeCommodity : commodityBasedflowVariablesMap) {
		INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_MOVES_SRC1,
				nodeCommodity.first);
		generateMoveCommodityFromSourceConstraint(nodeCommodity.second);
	}
}

void CPLEX_LP_IMSTSolver_v3::generateMoveCommodityFromSourceConstraint(
	...
	for (auto& toSourceEdge : edgeVariableMapForCommodity) {
		if (isEdgeToSourceExists(toSourceEdge.second)) {
			INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_MOVES_OUT_TO_SRC,
	...
	for (auto& fromSourceEdge : edgeVariableMapForCommodity.at(initialVertexIdx)) {
		INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_MOVES_FROM_SRC_OUT,
...
void CPLEX_LP_IMSTSolver_v3::generatePassCommodityConstraints() {
	INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_PASS_MOVES,
			initialVertexIdx);

	for (auto& nodeCommodity : commodityBasedflowVariablesMap) {
		INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_PASS_MOVES1,
				nodeCommodity.first);
...
void CPLEX_LP_IMSTSolver_v3::generatePassCommodityConstraint(
		IloFlowVarMap& edgeVariableMapForCommodity,
	...
	for (auto& targetVertexIdx : edgeVariableMapForCommodity) {
		if (targetVertexIdx.first != nodeCommodityIdx
			...
			INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_PASS_IN_NODE,
...
void CPLEX_LP_IMSTSolver_v3::generatePassCommodity(
	...
	for (auto& toTransitNodeEdge : edgeVariableMapForCommodity) {
		if (isEdgeExists(toTransitNodeEdge.second, transitNodeIdx)) {
			INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_PASS_IN_TO_NODE,
	...
	for (auto& fromTransitNodeEdge : edgeVariableMapForCommodity.at(
			transitNodeIdx)) {
		INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_PASS_FROM_NODE_OUT,
...
void CPLEX_LP_IMSTSolver_v3::generateStoreCommodityConstraints() {
	INFO_NOARG(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_STORE);

	for (auto& nodeCommodity : commodityBasedflowVariablesMap) {
		INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_STORE1,
				nodeCommodity.first);
...
void CPLEX_LP_IMSTSolver_v3::generateStoreCommodityConstraint(
		IloFlowVarMap& edgeVariableMapForCommodity,
		VertexIdx nodeCommodityIdx) {
	...
	for (auto& toCommodityEdge : edgeVariableMapForCommodity) {
		if (toCommodityEdge.second.count(nodeCommodityIdx) == 1) {
			INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_STORE_TO_NODE,
	...
	for (auto& fromCommodityEdge : edgeVariableMapForCommodity.at(
			nodeCommodityIdx)) {
		INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_STORE_FROM_NODE,
...
void CPLEX_LP_IMSTSolver_v3::generateFlowConstraints() {
	INFO_NOARG(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_FLOW_CONSTRAINTS);
				...
				model.add(
						targetVertex.second
								<= getEdgeVariable(sourceVertex.first,
										targetVertex.first));
				INFO(logger,
						LogBundleKey::CPLPIMST3_BUILD_MODEL_FLOW_CONSTRAINTS1,
...
void CPLEX_LP_IMSTSolver_v3::generateMSTConstraint() {
	INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_MST_CONSTRAINT,
...
void CPLEX_LP_IMSTSolver_v3::generateIncrementalSolutionConstraints(
		IncrementalParam k) {
	IloExpr incFormula(env);
	EdgeIF* edge { };
	VertexIdx sourceVertexIdx { };
	VertexIdx targetVertexIdx { };
	INFO_NOARG(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_INC_CONSTRTAINT);
	...
	while (graph->hasNextEdge(Connectivity::CONNECTED, Visibility::VISIBLE)) {
		...
		if (baseLPMSTSolution[sourceVertexIdx][targetVertexIdx] == true) {
			incFormula += getEdgeVariable(sourceVertexIdx, targetVertexIdx)
					+ getEdgeVariable(targetVertexIdx, sourceVertexIdx);
		}
	}
...
void CPLEX_LP_IMSTSolver_v3::generateGoal() {
	IloExpr goalFormula(env);
	IloInt i { };
	IloInt varArraySize { };
	EdgeIF* edge { };

	INFO_NOARG(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_GOAL_INCR_CONSTRAINT);

	graph->beginEdge();
	while (graph->hasNextEdge(Visibility::VISIBLE)) {
		edge = graph->nextEdge();
		INFO(logger, LogBundleKey::CPLPIMST3_BUILD_MODEL_GOAL_INCR_ADD,
...
void CPLEX_LP_IMSTSolver_v3::createUndirectedEdgeVariables(
		GraphIF * const graph) {
	createUndirectedEdgeVariables(graph, EDGE_VAR_DEFAULT_TYPE);
}

void CPLEX_LP_IMSTSolver_v3::createUndirectedEdgeVariables(
		GraphIF * const graph, IloNumVar::Type edgeVariablesType) {
	EdgeIdx edgeId { };
	VertexIdx sourceVertexIdx { };
	VertexIdx targetVertexIdx { };
	EdgeIF* edge { };
	...
	INFO(logger, LogBundleKey::CPLPIMST3_EDGE_VAR_GEN,
	...
	edgeVariableArray = IloNumVarArray(env,
			2 * graph->getNumberOfEdges(Visibility::VISIBLE), -1, 1,
			edgeVariablesType);
	...
	while (graph->hasNextEdge(edgeIterator, Visibility::VISIBLE)) {
		...
		INFO(logger, LogBundleKey::CPLPIMST3_EDGE_VAR_ADD,
...
EdgeSetIF* CPLEX_LP_IMSTSolver_v3::LP_Solve() throw (IloException) {
	EdgeSetIF* solution { };
	cplex.extract(model);

	INFO_NOARG(logger, LogBundleKey::CPLPIMST3_SOLVE_INIT);

	if (cplex.solve()) {
		INFO(logger, LogBundleKey::CPLPIMST3_SOLUTION_FOUND,
		...
		while (graph->hasNextEdge(Visibility::VISIBLE)) {
			...
			/*std::cout << "y_{" << sourceVertexIdx << "," << targetVertexIdx
			 << "} = z^{+}_{" << sourceVertexIdx << ","
			 << targetVertexIdx << "} - z^{-}_{" << sourceVertexIdx
			 << "," << targetVertexIdx << "} + x_{" << sourceVertexIdx
			 << "," << targetVertexIdx << "} = "
			 << cplex.getValue(
			 ...
			 targetVertexIdx).first) << std::endl;
			 std::cout << "y_{" << targetVertexIdx << "," << sourceVertexIdx
			 << "} = z^{+}_{" << targetVertexIdx << ","
			 << sourceVertexIdx << "} - z^{-}_{" << targetVertexIdx
			 << "," << sourceVertexIdx << "} + x_{" << sourceVertexIdx
			 << "," << targetVertexIdx << "} = "
			 << cplex.getValue(
			 edgeAddVariableMap.at(targetVertexIdx).at(
			 sourceVertexIdx).first) << " - "
			 << cplex.getValue(
			 edgeDropVariableMap.at(targetVertexIdx).at(
			 sourceVertexIdx).first) << " + "
			 << baseLPMSTSolution.at(sourceVertexIdx).at(targetVertexIdx)
			 << " = "
			 << cplex.getValue(
			 edgeVariableMap.at(targetVertexIdx).at(
			 sourceVertexIdx).first) << std::endl;*/

		}

		solution = transformSolutionToEdgeSet();
	} else {
		WARN_NOARG(logger, LogBundleKey::CPLPIMST3_SOLVE_NO_SOLUTION);
	}
	env.end();
	return solution;
}

void CPLEX_LP_IMSTSolver_v3::generateFlowVariables() {
	EdgeIdx edgeIdCount { };
		...
		while (graph->hasNextVertex(Visibility::VISIBLE)) {
			...
			for (auto& sourceVertex : edgeVariableMap) {
				...
				for (auto& targetVertex : sourceVertex.second) {
					...
					INFO(logger, LogBundleKey::CPLPIMST3_FLOW_GEN_COMMODITY_VAR,
...
IloNumVar CPLEX_LP_IMSTSolver_v3::getFlowVariable(
...
CPLEX_LP_IMSTSolver_v3::CPLEX_LP_IMSTSolver_v3(
...
CPLEX_LP_IMSTSolver_v3::CPLEX_LP_IMSTSolver_v3(GraphIF* const graph,
		EdgeSetIF * baseSolution, IloNumVar::Type edgeVariablesType) :
		CPLEX_LP_IMSTSolver_v3(nullptr, graph, baseSolution, edgeVariablesType) {
...
CPLEX_LP_IMSTSolver_v3::CPLEX_LP_IMSTSolver_v3(
		...
		CPLEX_LP_IMSTSolver_v3(mstSolver, graph, baseSolution,
				EDGE_VAR_DEFAULT_TYPE) {

}

CPLEX_LP_IMSTSolver_v3::CPLEX_LP_IMSTSolver_v3(GraphIF* const graph,
		EdgeSetIF * baseSolution) :
		CPLEX_LP_IMSTSolver_v3(nullptr, graph, baseSolution,
				EDGE_VAR_DEFAULT_TYPE) {
}

CPLEX_LP_IMSTSolver_v3::CPLEX_LP_IMSTSolver_v3(
		CPLEX_LP_MSTSolverIF* const mstSolver, GraphIF* const graph) :
		CPLEX_LP_IMSTSolver_v3(mstSolver, graph, nullptr, EDGE_VAR_DEFAULT_TYPE) {
}

CPLEX_LP_IMSTSolver_v3::CPLEX_LP_IMSTSolver_v3(GraphIF* const graph) :
		CPLEX_LP_IMSTSolver_v3(nullptr, graph, nullptr, EDGE_VAR_DEFAULT_TYPE) {
}
